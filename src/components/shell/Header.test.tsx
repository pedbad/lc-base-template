...
describe('Header', () => {
  test('renders exactly one header>nav labelled "Main navigation"', () => {
    const html = renderToStaticMarkup(<Header sections={DEFAULT_SECTIONS} />);
  ...
  test('renders a skip-to-main title link pointing at #content', () => {
    const html = renderToStaticMarkup(<Header sections={DEFAULT_SECTIONS} siteTitle="My Course" />);
  ...
  test('marks the active section link with aria-current', () => {
    const html = renderToStaticMarkup(
      <Header sections={DEFAULT_SECTIONS} activeSectionId="grammar" />,
    );
  ...
  test('renders an optional theme-toggle slot inside the nav', () => {
    const html = renderToStaticMarkup(
      <Header sections={DEFAULT_SECTIONS} themeToggle={<span data-testid="toggle-slot" />} />,
    );
