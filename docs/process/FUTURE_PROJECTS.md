...
18. Do not ship query-only content routing as the primary URL strategy when SEO/discoverability matters; use path slugs as canonical routes. (See #26 — in new projects do not even keep a numeric/query *resolver* as a backward-compat fallback; slug paths are the only route.)
